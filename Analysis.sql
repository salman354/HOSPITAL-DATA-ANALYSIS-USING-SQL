...
-- To fetch the all column from patient_data table
...
SELECT
    ethnicity,
    ROUND(COUNT(CASE WHEN hospital_death = 1 THEN 1 END) * 100 / (SELECT COUNT(*) FROM patient_data), 2) AS death_percentage
FROM patient_data
GROUP BY ethnicity;
